...
const { makePlan, slotAt } = require('../../lib/planner');
const { computeTarget, shouldSend, goalFollowUp } = require('../../lib/controller');
const SurplusTracker = require('../../lib/surplus');
const { SolarForecast } = require('../../lib/forecast');
...
class ZendureDevice extends Homey.Device {
  ...
  createClients() {
    this.zendure = new ZendureClient(this.getSetting('zendure_ip'), this.getStoreValue('sn'));
  ...
  get cfg() {
    ...
    const batteries = Math.max(1, this.driver.getDevices().length);
    return {
  ...
  async onStrategyChanged(strategy) {
    ...
    this.driver.strategyChangedTrigger.trigger(this, { strategy }).catch(this.error);
  ...
  updatePlan() {
    ...
    if (this.getCapabilityValue('battery_plan') !== action) {
      ...
      this.driver.planChangedTrigger.trigger(this, { action, price: price || 0 }).catch(this.error);
  ...
  async tick() {
    ...
    const cfg = this.cfg;
    try {
      const [zResult, pResult] = await Promise.allSettled([this.zendure.report(), this.p1.read()]);
      if (zResult.status === 'rejected') throw zResult.reason; // cannot control without the battery

      const bat = ZendureClient.parse(zResult.value);
      ...
      if (meter) {
        ...
        this.p1Failures += 1;
        this.error(`P1 read failed (${this.p1Failures}x):`, pResult.reason.message);
      }
      ...
      if (!cfg.demo && !this.stopped && shouldSend(target, this.lastCommand, now, cfg)) {
        await this.zendure.setPower(target);
  ...
  evaluateSurplusTriggers() {
    ...
    this.driver.surplusAboveTrigger.trigger(this, tokens, state).catch(this.error);
    this.driver.surplusBelowTrigger.trigger(this, tokens, state).catch(this.error);
  }
  ...
  async onDemoChanged(enabled) {
    ...
    this.lastCommand = null;
    this.driver.demoChangedTrigger.trigger(this, { demo: enabled }).catch(this.error);
    this.scheduleTick(200);
  }
  ...
  async onSettings({ newSettings, changedKeys }) {
    ZendureDevice.validateSettings(newSettings);
  ...
  static validateSettings(s) {
    ...
    if (!ipRe.test(s.zendure_ip) || !ipRe.test(s.p1_ip)) {
  ...
  afterSettingsChanged(changedKeys, newSettings) {
    this.lastCommand = null;
    if (changedKeys.some((k) => ['zendure_ip', 'p1_ip', 'p1_token'].includes(k))) this.createClients();
  ...
  async applySettings(changes) {
    const schema = new Map(this.homey.app.settingsSchema().map((f) => [f.id, f]));
    const clean = {};
  ...
  async onDeleted() {
    this.cleanup();
    // leave the battery idle (in demo mode it keeps its own program)
    if (!this.getSetting('demo_mode')) await this.zendure.setPower(0).catch(this.error);
    this.homey.setTimeout(() => this.homey.app.updateWebServer(), 1000);
  }

  async onUninit() {
    this.cleanup();
    // best effort: leave the battery idle when the app stops (not in demo mode)
    if (!this.getSetting('demo_mode')) await this.zendure.setPower(0).catch(this.error);
  }
