 ...
 *   GET  /api/log          last log lines (JSON)
 *   GET  /api/diagnostics  diagnostics + full log as a downloadable text file
 */
class WebServer {

  constructor({
    page, getDevices, getSchema, getDiagnostics = () => '', getRecentLog = () => [], log = () => {}, error = () => {},
  }) {
    this.page = page;
    this.getDiagnostics = getDiagnostics;
    this.getRecentLog = getRecentLog;
  ...
  async handle(req, res) {
    ...
    if (req.method === 'GET' && url.pathname === '/api/log') {
      ...
      this.send(res, 200, { lines: this.getRecentLog(limit) });
      return;
    }
    if (req.method === 'GET' && url.pathname === '/api/diagnostics') {
      const stamp = new Date().toISOString().slice(0, 16).replace(/[-:T]/g, '');
      this.send(res, 200, this.getDiagnostics(), 'text/plain; charset=utf-8', {
        'Content-Disposition': `attachment; filename="batterij-regeling-diagnose-${stamp}.txt"`,
      });
      return;
    }
    if (req.method === 'GET' && url.pathname === '/api/settings') {
      ...
      const fields = this.getSchema(device).map((f) => ({
        ...f,
