...
class SolarForecast {
  ...
  /**
   * Fetch the forecast for one or more roof planes (e.g. east + west) and sum them.
   * The free API allows one plane per call, so each plane is a separate request.
   */
  async refresh({ lat, lon, planes }) {
    const results = [];
    for (const { tilt, azimuth, kwp } of planes) {
      const url = `${API}/${lat.toFixed(4)}/${lon.toFixed(4)}/${tilt}/${azimuth}/${kwp}`;
      const json = await requestJson(url, { timeout: 15000 });
      results.push(json.result || {});
    }
    this.setPlanes(results);
    this.fetchedAt = Date.now();
    this.log(`Solar forecast: ${planes.length} plane(s), ${this.points.length} points, today ${this.energyToday().toFixed(1)} kWh`);
  }

  setWatts(watts) {
    this.setPlanes([watts]);
  }

  /** Sum several forecast.solar results; each plane is interpolated on the union of all timestamps. */
  setPlanes(list) {
    const planes = list.map((watts) => Object.entries(watts)
      .map(([k, w]) => ({ t: localToUtc(k, this.timezone), w: Number(w) }))
      .sort((a, b) => a.t - b.t));
    const times = [...new Set(planes.flat().map((p) => p.t))].sort((a, b) => a - b);
    this.points = times.map((t) => ({ t, w: planes.reduce((sum, p) => sum + interpolate(p, t), 0) }));
  }
  ...
  wattsAt(t) {
    ...
    return interpolate(this.points, t);
  }
