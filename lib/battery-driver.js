...
class BatteryDriver extends Homey.Driver {
  ...
  async onPair(session) {
    ...
    session.setHandler('scan', async () => {
      ...
      const { modbus } = this.brand();
      const result = await scanNetwork(ownIp, { zendure: !modbus, modbusPort: modbus ? 502 : null });
      return {
    ...
    session.setHandler('configure', async ({
      ...
      if (!IP_RE.test(batteryIp)) throw new Error('Ongeldig IP-adres van de batterij');
      if (!IP_RE.test(p1Ip)) throw new Error('Ongeldig HomeWizard P1 IP-adres');
      const { modbus, label } = this.brand();
      ...
      const settings = { battery_ip: batteryIp, p1_ip: p1Ip };
      if (modbus) {
