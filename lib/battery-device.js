...
// forecast.solar allows 12 calls per hour per IP address: retry a failed fetch after 10 minutes
const FORECAST_RETRY_MS = 10 * 60 * 1000;
...
class BatteryDevice extends Homey.Device {
  ...
  logDataLine({
    ...
    this.logbookAdd('data', [[
      ...
      ...(meter && meter.realGridPower !== undefined ? [`echt=${meter.realGridPower}W`] : []),
      `gewenst=${target}W`,
  ...
  /** Battery power capability; the simulator uses a sub-capability so Homey Energy does not count it. */
  get powerCapability() {
    return this.hasCapability('measure_power') ? 'measure_power' : 'measure_power.battery';
  }

  /** State of charge capability; the simulator uses battery_soc (see drivers/simulator/device.js). */
  get socCapability() {
  ...
  get cfg() {
    ...
    return {
      ...
      forecastEnabled: s.forecast_enabled && this.solarPlanes().length > 0,
      demo: s.demo_mode === true,
    };
  }
  ...
  /** Roof planes with panels (plane 2 is optional, e.g. east + west). */
  solarPlanes() {
    const s = this.getSettings();
    return [
      { kwp: s.pv_kwp, tilt: s.pv_tilt, azimuth: s.pv_azimuth },
      { kwp: s.pv2_kwp, tilt: s.pv2_tilt, azimuth: s.pv2_azimuth },
    ].filter((p) => p.kwp > 0);
  }

  async refreshForecast() {
    if (this.forecastRetry) this.homey.clearTimeout(this.forecastRetry);
    this.forecastRetry = null;
    const s = this.getSettings();
    const planes = this.solarPlanes();
    const skip = !s.forecast_enabled ? 'off'
      : !planes.length ? 'Solar forecast is on, but no roof plane has a power (kWp) filled in' : null;
    // log a skip once per cause, not every hour
    if (skip && skip !== 'off' && skip !== this.forecastSkip) this.log(skip);
    this.forecastSkip = skip;
    if (skip) return;
    const lat = this.homey.geolocation.getLatitude();
    const lon = this.homey.geolocation.getLongitude();
    try {
      if (!Number.isFinite(lat) || !Number.isFinite(lon)) throw new Error('location of Homey unknown');
      await this.forecast.refresh({ lat, lon, planes });
      await this.setCapabilityValue('solar_forecast_today', Math.round(this.forecast.energyToday() * 10) / 10);
      this.updatePlan();
    } catch (err) {
      this.error(`Solar forecast failed (retry in ${FORECAST_RETRY_MS / 60000} min):`, err.message);
      if (!this.stopped) this.forecastRetry = this.homey.setTimeout(() => this.refreshForecast(), FORECAST_RETRY_MS);
    }
  }
  ...
  getWidgetData() {
    return {
      name: this.getName(),
      soc: this.getCapabilityValue(this.socCapability),
      power: this.getCapabilityValue(this.powerCapability),
  ...
  async tick() {
    ...
    try {
      ...
      let meter = pResult.status === 'fulfilled' ? pResult.value : null;
      if (meter) {
        this.p1Failures = 0;
      } else {
        this.p1Failures += 1;
      ...
      // Simulated battery: the P1 meter measures the house without it. The control and the surplus
      // work with the grid power as it would be with the battery.
      if (meter && this.battery.simulated) {
        this.onSimulatedReading(bat, meter.gridPower);
        meter = { ...meter, realGridPower: meter.gridPower, gridPower: meter.gridPower - bat.batteryPower };
      }

      this.updateMeters(bat.batteryPower, bat.soc);
      const updates = [
        bat.soc !== null ? this.setCapabilityValue(this.socCapability, bat.soc) : null,
        // Homey convention for home batteries: positive = charging
        this.setCapabilityValue(this.powerCapability, -bat.batteryPower),
      ];
      if (meter) {
      ...
      if (meter) this.afterControl(bat, meter);
  ...
  /** Simulator only: bookkeeping of one reading (simulated battery + real P1 power). */
  onSimulatedReading() {}

  /** Simulator only: one control round done (for the live chart). */
  afterControl() {}
  ...
  statusText(target, reason, demo = false) {
    ...
    if (this.battery && this.battery.simulated) return `Simulatie – ${what.toLowerCase()} · ${reason}`;
    return `${what} · ${reason}`;
  }
  ...
  static validateSettings(s) {
    ...
    // the simulator has no battery IP address
    if ((s.battery_ip !== undefined && !ipRe.test(s.battery_ip)) || !ipRe.test(s.p1_ip)) {
  ...
  afterSettingsChanged(changedKeys, newSettings) {
    ...
    if (changedKeys.some((k) => ['battery_ip', 'p1_ip', 'p1_token'].includes(k) || k.startsWith('modbus_')
      || k.startsWith('marstek_') || k === 'invert_power' || k.startsWith('sim_'))) this.createClients();
    if (changedKeys.includes('entsoe_token')) {
      this.homey.app.prices.setEntsoeToken(this.getSetting('entsoe_token'));
      this.homey.app.prices.refresh().catch(this.error);
    }
    if (changedKeys.some((k) => k.startsWith('pv') || k === 'forecast_enabled')) this.refreshForecast();
  ...
  getWebData() {
    ...
    return {
      ...
      simulation: this.simulationData ? this.simulationData() : null,
